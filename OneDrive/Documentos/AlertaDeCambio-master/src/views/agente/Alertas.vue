...
const buscarCotacaoAtual = async (alerta: any) => {
  try {
    ...
    let cotacaoFinal: number

    // Se for remessas, usa a cotação comercial diretamente
    if (alerta.produto === 'remessas') {
      cotacaoFinal = cotacoesService.cotacoes.value[alerta.moeda] || 0
      console.log(`Cotação comercial para ${alerta.moeda} (remessas):`, cotacaoFinal)
    } else {
      // Se for turismo, aplica a taxa
      cotacaoFinal = cotacoesService.calcularCotacaoFinal(alerta.moeda)
      console.log(`Cotação turismo para ${alerta.moeda}:`, cotacaoFinal)
    }
...
// Formatar moeda com precisão baseada no produto
const formatarMoeda = (valor: number, produto?: string): string => {
          ...
          <div class="space-y-4">
            <div>
              ...
              <div class="mt-1 ml-6 space-y-1">
                <p class="text-sm text-gray-900">{{ formatarMoeda(alerta.cotacaoAlvo, alerta.produto) }}</p>
                <div class="flex items-center gap-2">
                  <p class="text-sm text-gray-500">Atual: 
                    <span :class="[
                      ...
                      {{ formatarMoeda(cotacoesAtuais[alerta.id], alerta.produto) }}
                    </span>
                  </p>
                  ...
                  <div class="grid grid-cols-1 sm:grid-cols-12 gap-4 mt-4">
                    ...
                    <div class="sm:col-span-3">
                      ...
                      <div class="block w-full rounded-lg border-0 py-2.5 px-3 bg-gray-50 text-gray-900 font-medium sm:text-sm sm:leading-6 h-[42px] ring-1 ring-inset ring-gray-300">
                        {{ cotacaoAtual ? formatarMoeda(cotacaoAtual, editingAlerta.produto) : '' }}
                      </div>
                    </div>
